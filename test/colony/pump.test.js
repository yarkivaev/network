...
describe('pump', () => {
  test('returns positive total flow', () => {
    ...
    const ids = col.modules().map((m) => m.identifier());
    assert.ok(pump(col, ids[0], ids[1], fakeFlow).total() > 0, 'total flow was not positive');
  });
  ...
  test('returns flow per edge', () => {
    ...
    const col = colony(obstacle(net, bernoulli(0, Math.random), new Map()), bernoulli(0, Math.random));
    const ids = col.modules().map((m) => m.identifier());
    assert.ok(pump(col, ids[0], ids[1], fakeFlow).flow().size > 0, 'no flow edges returned');
  });
  ...
  test('returns bottleneck edges', () => {
    ...
    const col = colony(obstacle(net, bernoulli(0, Math.random), new Map()), bernoulli(0, Math.random));
    const ids = col.modules().map((m) => m.identifier());
    assert.ok(pump(col, ids[0], ids[1], fakeFlow).bottlenecks().size > 0, 'no bottlenecks returned');
  });
});
