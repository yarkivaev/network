...
describe('colony', () => {
  ...
  test('returns passable roads excluding obstacles', () => {
    ...
    const col = colony(obstacle(net, bernoulli(1, () => 0), new Map()), bernoulli(0, Math.random));
  ...
  test('destroy removes a module and its edges', () => {
    const net = triangulation(5, normal(0, 10, Math.random), unit).network();
    const dist = bernoulli(0, Math.random);
    const col = colony(obstacle(net, dist, new Map()), dist);
    const destroyed = col.destroy(col.modules()[2]);
    assert.equal(destroyed.modules().length, 4, 'module was not removed');
  });

  test('sever removes a road between two modules', () => {
    const net = triangulation(4, normal(0, 10, Math.random), unit).network();
    const dist = bernoulli(0, Math.random);
    const col = colony(obstacle(net, dist, new Map()), dist);
    const before = col.network().edges().items().length;
    const severed = col.sever(0, 1);
    assert.equal(severed.network().edges().items().length, before - 2, 'edge pair was not removed');
  });
});
